...
# Usage:
#   python process_video_data3.py <video.mp4> --out <output_dir>
# -----------------------------------
...
def process(video_path, out_dir):
    ...
    # base lock (now bbox bottom mid-point)
    base_cx_locked = None
    base_cy_locked = None
    ...
    while True:
        ...
        if cnts:
            ...
            cv2.putText(frame, f"Bend ang: {bend_deg_smooth:+.1f} deg",
            ...
            # if fallback_used:
            #     cv2.putText(frame, "TIP FALLBACK",
            #                 (int(min(pts[:,0])), label_y + 44),
    ...
    with open(csv_frames, "w", newline="") as f:
        ...
        # keep times if you want them later:
        _ = (i_lin, i_rad, i_area)
    ...
    with open(csv_summary, "a", newline="") as f:
        ...
        wr.writerow(dict(
            ...
            max_lin_time=(max_lin_frame*dt),
            max_rad_mm=max_rad_mm, 
            max_rad_time=(max_rad_frame*dt),
            max_area_mm=max_area_mm, 
            max_area_time=(max_area_frame*dt),
            max_bend_deg=max_bending_angle,
            max_bend_mm=max_bending_dist,
            max_bend_time=(max_bend_frame*dt),
            mm_per_px=scale
        ))
